#インプットファイルは二つ。リストと表を比較して、表の中からリストに一致する行を取ってくる。
#1列目が遺伝子名、2列目以降が発現量、1行目がヘッダとなっているファイルを想定。
#例えば、遺伝子発現量の表から、DEGのリストにあるものだけを取ってくる。
#コマンドラインから、"python3 select-list1.py ファイル名 リスト名 出力ファイルの名前"と入力して実行。
